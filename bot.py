"""
EAS Arena — Discord Bot (Consolidated)
=======================================
Single-file bot that combines all EAS Arena bot features:

  Prefix commands (owner only):
    !addkills @user <kills>                  — Add kills retroactively (10-second review countdown)
    !scrimrollback @user1 @user2 ... <kills> — Remove kills from multiple players (20-second countdown)
...
async def _run_countdown(
    ...
    embed: discord.Embed,
    seconds: int,
    countdown_field_name: str,
) -> None:
    """
    Edit *message* every second, updating the embed field named
    *countdown_field_name* with the remaining seconds.
    Stops when the counter reaches 0.
    """
    for remaining in range(seconds, 0, -1):
        # Update the countdown field in-place
        for i, field in enumerate(embed.fields):
            if field.name == countdown_field_name:
                embed.set_field_at(
                    i,
                    name=countdown_field_name,
                    value=f"⏳ Applying in **{remaining}s**…",
                    inline=field.inline,
                )
                break
        try:
...
async def _send_premium_sync(
    ...
    log.info(
        "Sending premium sync for user %s: premium=%s, granted_at=%s",
        user_id,
        premium,
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if resp.status == 200 and data.get("ok"):
                    log.info("✅ Premium sync success for user %s (premium=%s)", user_id, premium)
                    return True, f"Database updated (premium={premium})"
                else:
                    err = data.get("error", f"HTTP {resp.status}")
                    log.warning("❌ Premium sync failed for user %s: %s", user_id, err)
                    return False, f"Website sync failed: {err}"
    except aiohttp.ClientError as exc:
        log.error("Network error syncing premium for user %s: %s", user_id, exc)
...
async def on_guild_member_update(
    ...
    if has_role:
        log.info("🎉 User %s (%s) GAINED premium role %s", user_id, user_name, PREMIUM_ROLE_ID)
        action_label = "GRANTED"
    else:
        log.info("⚠️  User %s (%s) LOST premium role %s", user_id, user_name, PREMIUM_ROLE_ID)
...
async def _log_premium_sync(
    ...
    try:
        ...
        log.warning("Failed to send premium sync log message: %s", exc)


# ---------------------------------------------------------------------------
# !addkills @user <kills>  (owner only, 10-second countdown)
# ---------------------------------------------------------------------------
...
async def addkills(
    ...
    Shows a 10-second review countdown before applying the change.
    Owner-only command.
    """
    # --- Permission check ---
    ...
    get_status, get_data = await _api_get("/api/admin/players", {"userId": user_id})
    ...
    # --- Send countdown embed (10 seconds) ---
    countdown_field = "Status"
    embed = discord.Embed(
        title="⏱️ Add Kills — Review",
        description=(
            "Review the details below. The change will be applied automatically "
            "after the countdown."
        ),
        color=discord.Color.orange(),
    )
    embed.add_field(name="Player",      value=f"{member.mention} (`{user_id}`)", inline=False)
    embed.add_field(name="Kills to Add", value=str(kills),                       inline=True)
    embed.add_field(name="Current Total", value=str(current_kills),              inline=True)
    ...
    embed.add_field(name=countdown_field, value="⏳ Applying in **10s**…",       inline=False)
    embed.set_thumbnail(url=member.display_avatar.url)
    embed.set_footer(text="This action cannot be cancelled once the countdown ends.")

    msg = await ctx.reply(embed=embed, mention_author=False)

    # Run the live countdown
    await _run_countdown(msg, embed, 10, countdown_field)
    ...
    if status == 200 and data.get("ok"):
        ...
        result_embed.add_field(name="Player",      value=f"{member.mention} (`{user_id}`)",                    inline=False)
        result_embed.add_field(name="Kills Added", value=str(kills),                                           inline=True)
        result_embed.add_field(name="Old Total",   value=str(current_kills),                                   inline=True)
        result_embed.add_field(name="New Total",   value=str(new_kills),                                       inline=True)
        result_embed.add_field(name="Added By",    value=str(ctx.author),                                      inline=True)
        result_embed.add_field(name="Timestamp",   value=f"`{timestamp[:19].replace('T', ' ')} UTC`",          inline=True)
        result_embed.set_footer(text="History entry written to player record.")
        result_embed.set_thumbnail(url=member.display_avatar.url)
        await msg.edit(embed=result_embed)

        log.info(
            "✅ Added %d kills to %s (%s): %d → %d",
...
# ---------------------------------------------------------------------------
# !scrimrollback @user1 @user2 ... <kills>  (owner only, 20-second countdown)
# ---------------------------------------------------------------------------
...
async def scrimrollback(
    ...
    *args: str,
    ...
    Shows a 20-second review countdown before applying the changes.
    Owner-only command.
    """
    # --- Permission check ---
    if not is_owner(ctx.author.id):
    ...
    # --- Send countdown embed (20 seconds) ---
    countdown_field = "Status"
    embed = discord.Embed(
        title="⏱️ Scrim Rollback — Review",
        description=(
            "Review the details below. The rollback will be applied automatically "
            "after the countdown."
        ),
        color=discord.Color.orange(),
    )
    ...
    embed.add_field(name="Kills to Remove", value=str(kills_to_remove), inline=True)
    embed.add_field(name="Player Count",    value=str(len(members)),    inline=True)
    embed.add_field(name="Requested By",    value=str(ctx.author),      inline=True)
    embed.add_field(name=countdown_field,   value="⏳ Applying in **20s**…", inline=False)
    embed.set_footer(text="This action cannot be cancelled once the countdown ends.")

    msg = await ctx.reply(embed=embed, mention_author=False)

    # Run the live countdown
    await _run_countdown(msg, embed, 20, countdown_field)
    ...
    for member in members:
        ...
        if post_status == 200 and post_data.get("ok"):
            ...
            log.info(
                "✅ Scrim rollback: removed %d kills from %s (%s): %d → %d",
                kills_to_remove, member.display_name, user_id, current_kills, new_kills,
            )
        else:
            ...
            log.warning(
                "❌ Scrim rollback failed for %s (%s): %s",
                member.display_name, user_id, err,
            )
    ...
    result_embed.add_field(name="Rolled Back By", value=str(ctx.author),                              inline=True)
    result_embed.add_field(name="Timestamp",      value=f"`{timestamp[:19].replace('T', ' ')} UTC`", inline=True)
    result_embed.set_footer(text="History entries written to each updated player record.")

    await msg.edit(embed=result_embed)


# ---------------------------------------------------------------------------
# !premiumsync @user  (owner only)
# ---------------------------------------------------------------------------
...
async def premiumsync(
    ...
    log.info(
        "!premiumsync triggered by %s for user %s (%s): has_role=%s",
...
async def list_commands(ctx: commands.Context) -> None:
    ...
    embed.add_field(
        ...
        value=(
            ...
            "Shows a **10-second** review countdown before applying.\n\n"
            ...
            "Shows a **20-second** review countdown before applying.\n\n"
    ...
    embed.set_footer(text="EAS Arena Bot  •  Use !commands to see this list at any time")

    await ctx.reply(embed=embed, mention_author=False)


# ---------------------------------------------------------------------------
# Slash command group: /giveawaycode  (owner only)
# ---------------------------------------------------------------------------


giveawaycode_group = app_commands.Group(
    name="giveawaycode",
    description="Manage EAS Arena Premium giveaway codes (owner only).",
)


@giveawaycode_group.command(name="create", description="Create a new Premium giveaway code.")
...
@giveawaycode_group.command(name="list", description="List all Premium giveaway codes.")
@owner_only_check()
...
@giveawaycode_group.command(name="disable", description="Disable a giveaway code.")
...
@giveawaycode_group.command(name="info", description="Get details about a specific giveaway code.")
...
bot.tree.add_command(giveawaycode_group)
...
async def on_command_error(ctx: commands.Context, error: commands.CommandError) -> None:
    if isinstance(error, commands.MemberNotFound):
        ...
        await ctx.reply(
            f"❌ Invalid argument: `{error}`. Check the command usage with `!commands`.",
            mention_author=False,
        )
    elif isinstance(error, commands.MissingRequiredArgument):
        await ctx.reply(
            f"❌ Missing required argument: `{error.param.name}`. "
            "Check the command usage with `!commands`.",
            mention_author=False,
        )
